---
title: "The effect of class-type and schools on math scores of first-grade students"
output:
  html_document:
    df_print: paged
    fig_caption: yes
...
Github repo link:https://github.com/maoqunqun/STA207winter2020-project2-team-3.git

```{r,includ=F}
library(foreign)
STAR_full<-read.spss("F:\\2020 winter\\sta207\\project2\\STAR_Students.sav",to.data.frame = TRUE)
```
# Introduction 

  In order to explore the effects of class type on student achievement, a four-year longitudinal study called Student/Teacher Achievement Ratio (STAR) was conducted by the State Department of Education (Achilles, 2008). Around 6600 students and 339 teachers were randomly assigned to three different class types for first-year students: small class, regular class, and regular-with-aide class. In this paper, we use teachers as the unit to investigate the total SAT math scale score of the first-grade students and determine whether there is a difference in their math scores across different class types in different schools.
  ...
  There is evidence that different class types affect math scores of the first-grade student in previous work. This paper also considers the effect of different teachers and different schools and finds that both class type and school affect their math scores.
...
Figure 3 and figure 4 provides the box-plot of math scores’ distribution grouped by class type and school ID, respectively. In Figure 3, we can see that students tend to have higher math scores in small classes and lower math scores in regular class without aides. In Figure 4, generally, we can see that students have similar math scores in different schools, but there are still several schools showing lower math scores than other schools.
...
par(mfrow=c(1,2))
#boxplot
boxplot(g1tmathss~g1classtype,data=data_new,main="Figure 3: Boxplot of math scores grouped by class types",ylab="math score",xlab="class type",cex.main=1)
boxplot(g1tmathss~g1schid,data=data_new,main="Figure 4: Boxplot of math scores grouped by school id", ylab="math score",xlab="school ID",cex.main=1,las=2,cex=0.1,cex.axis=0.5)
```
...
* $Y_{ijk}$ denote the median of students' total SAT math score for each teacher.


* $\gamma_{ij}$ denote interaction between class type $i$ and school $j$.

* $\mu$ denote the mean of all total SAT math scores for 339 teachers.
...
* Hypothesis 1: class types does not affect math scores. The corresponding null and alternative hypotheses are:

    $H_0:\alpha_1=\alpha_2=\alpha_3=0 \  vs.\ H_a: not \ all\ \alpha_i \ equal \ to\  0\ (i=1,2,3).$ 

* Hypothesis 2: school does not affect math scores. The corresponding null and alternative hypotheses are:
...
Although we provide statistical evidence showing the impact of class type and school, there are still some possible problems that we can consider in the future work. As the main subject, we investigate the students' math achievement, and we use the median of students’ math scores as the response variable to represent the math achievement of students for each teacher. We decided to use the median for two reasons: Firstly, the median score can be regarded as a typical and sufficient statistic, which can present the overall math level of a class of students. In addition, when we use mean math scores as the response variable, the equal variance assumption was violated, thus in the future work, the non-parametric two-way ANOVA model can also be considered. Also, note that there is no significant interaction effect in this paper; we may also further consider the interaction term in the future.
